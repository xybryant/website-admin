var Pagination = {
    showPageCount: 7,
    init: function(obj, callback, topFlag) { this.bindListener(obj, callback, topFlag); },
    createHtml: function(pageIndex, recordCount, pageSize) {
        ...
        var html = [];
        if (pageCount > MaxCount) { pageCount = MaxCount; }
        if (pageIndex > pageCount - 1) { pageIndex = pageCount - 1; }
        html.push("<span class=\"total\">共" + recordCount + "条</span>");
        if (pageIndex > 0) { html.push("<span class=\"previous\"><a href=\"javascript:;\" page= " + (pageIndex - 1) + " data-rec=\"上一页\">&lt;</a></span>"); } else { html.push("<span class=\"disable previous\">&lt;</span>"); }
        if (pageCount <= showPageCount) {
            for (var i = 0; i < pageCount; i++) {
                if (pageIndex == i) { html.push("<span class=\"current num\">" + (i + 1) + "</span>"); } else { html.push("<span class=\"num\"><a href=\"javascript:;\"  page=" + i + ">" + (i + 1) + "</a></span>"); }
            }
        ...
        if (pageIndex < pageCount - 1) { html.push("<span class=\"next\"><a href=\"javascript:;\" page=" + (pageIndex + 1) + " data-rec=\"下一页\">&gt;</a></span>"); } else { html.push("<span class=\"disable next\">&gt;</span>"); }
    ...
    bindListener: function(obj, callback, topFlag) {
        ...
        obj.on("click", "a", function() {
            if (typeof callback === "function") { var index = $(this).attr("page");
            //if (typeof callback === "function") { var index = $(this).attr("page");
        ...
        obj.on("click", "button", function() {
            ...
            if (typeof callback === "function") {
                ...
                if (index == '') { pageInput.focus(); return false; }
        ...
        obj.on("keyup", "input", function(e) {
            switch (e.keyCode) {
                ...
                case 13:
                    changePage(); break;
                case 8:
                    $(e.target).attr('oldpage', $(e.target).val()); break;
                case 46:
                    $(e.target).attr('oldpage', $(e.target).val()); break;
                default:
                    validPage(); }
        ...
        function changePage() {
            ...
            if (page.trim() == "") { pageInput.focus(); return; }
            if (pattern.test(page)) { page = parseInt(page);
                maxPage = parseInt(maxPage); if (page > maxPage || page < 0) { pageInput.val("").focus(); return; } } else { pageInput.val("").focus(); return; }
        ...
        function validPage() {
            ...
            if (page.trim() == "") { pageInput.val(""); return; }
            if (!pattern.test(page)) { pageInput.val(oldpage); return; }
            var pageInt = parseInt(page);
            if (page.substr(0, 1) == '0') { pageInput.val(pageInt); }
    ...
    Page: function(obj, pageIndex, recordCount, pageSize) { obj.empty();
